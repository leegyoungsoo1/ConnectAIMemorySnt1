...
`python run_command.py --cmd "명령어"` 형식으로 사용. 예: `--cmd "python manage.py migrate"`, `--cmd "pytest"`

### 📝 `write_file` — 코드 파일 저장
`$code=@'...코드...'@; $code | python write_file.py --path "app/파일.py"` 형식으로 사용.

### 📖 `read_file` — 파일 읽기
`python read_file.py --path "app/models.py"` 형식으로 사용. 줄번호 포함 출력.
